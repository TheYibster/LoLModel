...
League of Legends(LoL) is a 5v5, player vs player, MOBA game. If you don't know what that means...great! It doesn't matter for the purposes of this model! At it's core, LoL is a game with two teams(Blue and Red) of five players, with each player piloting a unique champion, battling it out to capture the enemy's base (aka Destroy the Nexus).

![**(Visual 1.1)**](./images/Map.jpeg)

So what must a team do to win?
...
If this all sounds like gibberish, just think of objectives as a measurement of progress. The more objectives equals the more progress a team has made.
...
Basically, pretty much anything you do in LoL grants you gold. In simple terms, think of gold a a measurement of a team's power.
...
Basically, whenever you kill something, you gain experience. Again, think of experience as a team's growth.
...
Again, I will be  using this <a href="https://www.kaggle.com/datasets/bobbyscience/league-of-legends-diamond-ranked-games-10-min">dataset</a> from Kaggle by Yi Lan Ma.
...
As one can see, there are quite a few variables that are just additive inverses of another variable. This means that they have a correlation of -1. At the same time, there are also variables that are perfectly correlated meaning that they have a correlation of 1. This is a big problem because these perfectly correlated variables are basically measuring the same things in our data; we need to drop them!
...
With duplicates sorted out, let's also convert the factors into factor types in our data frame as they are currently numeric types.
...
Our data should be much cleaner and ready to be worked on, but let's take a look at the correlation matrix before we move on:
...
Upon first glance, there are a lot of variables; however, if we think back to when I introduced gold, experience, and objectives along with our correlation matrix, it's apparent that most of these variables are contributing to those three factors.

As reference, let's visualizes `blueKills` as a relation with blue team's Gold.
...
Great, a picture perfect linear relation(positive linear trend)! If this is not clear that more kills leads to more gold then I don't know what is. What about kills and experience?
...
Another positive linear relation! Similar to kills, many of our other variables express the same relation with gold, experience, and objectives; however, this also means that there are many highly correlated variable as we saw in the correlation plot, and we will keep this in mind.
...
After thinking about all of our variables, a majority of variation in our data can be explained with gold, experience, and objectives. Let's see how they affect a team's win one-by-one.
...
So this supports our theory that the outcome of a match of League of Legends is highly related to the gold, experience, and objectives that a team acquires. Now for the controversial question, which out of those three, is more important?

There is only so much a team can do in the first 10 minutes of a match; it would be much better if we know what to focus on for a higher chance of winning. Should the team focus on getting Gold? Or should they focus on taking Objectives? Or should the team thirst for blood and kill for experience?
...
Before we perform a split, there is something that we must address. We must stratify our data on `blueWins` in order to preserve the proportions of win-lose ratio present in our data. 
...
Interesting, it seems that our model is actually quite balanced in terms of win-lose ratio; however, we must still stratify by `blueWins` in order to keep this 50-50 ratio across our training and testing sets.

I will go ahead and perform the initial split of the data into 80-20 train-test set, and I will be stratifying the data on `blueWins`.
...
![Example of 5-fold](./images/k-fold.png)
...
Now that we have our folds, recipe, and split; we are ready to start feeding our data into machine learning models to begin the tuning process.

## Tuning the models

The models will be tuned in a separated R script; however the basic steps for our model tuning goes like this:

1. Set up the model by defining it's type, engine, and its mode (for us it is classification)
...
ROC curve is a graph showing the performance of a classification model at all classification thresholds. The ROC curve measures the True(sensitivity) and False(1 - specificity) positive rates.
...
![Time to Get to Work](./images/ornn.gif)
...
It looks like the Elastic Net model prefers a smaller penalty if the mixture is large and a greater penalty when mixture is small; but, what is the actual values of our hyperparameters?

```{r}
glm_best
```

The most optimal hyperparameters ended up being `penalty` = 0 and `mixture` = 1. This suggests that a pure lasso model fits our data the best!
...
Now we have two models fitted on our training data. However, this is only with our training data. How will these numbers hold up when we test our models with our testing set?

By using a set of never-seen before data, will our model continue to perform with high `roc_auc` values or will we find that our model has flaws such as overfitting to the training set?
...
Now 0.805 may not be the best, but it is quite reliable for the purposes of this project. So can this boosted tree model tell me what is the most important factor when it comes to winning a game of League of Legends?
...
So it seems that a team needs, on average, a 1000 gold lead at the 10-minutes mark is more likely to win. Also, it seems that the reverse is true as well, which makes sense; a team that is 1000 gold behind at the 10-minute mark is more likely to lose the match.

## What can be improved

League is a game that is constantly changing, a model done with an updated data set might be more accurate to today's League of Legends. One of the biggest problems when fitting our models was the overwhelming amount of linear dependence in predictors; in the future, I would look for other methods of dealing with linear dependence in our data rather than simply removing them. One approach that I have in mind is to use Principle Component Analysis to project the data onto eigenvectors before fitting. Finally, a different model may fit this data better; however considering that the best model was an Elastic Net(LASSO), I am unsure if a more complex model can do any better.

![](./images/dance.gif)

With that being said, thank you for reading this report. I hope it was somewhat insightful for League of Legends players and non-players alike.
